...
test('maps page has interactives maps', async ({ page }) => {
    ...
    const piafabecDiv = page.locator('#pokedex div').filter({ hasText: 'Piafabec' });
    const magnetiDiv = page.locator('#pokedex div').filter({ hasText: 'Magnéti' });
    await expect(bulbizarreDiv).toBeVisible();
    await expect(piafabecDiv).toBeVisible();
    await expect(magnetiDiv).toBeVisible();
    ...
    await expect(magnetiDiv).not.toBeVisible();
    ...
    await expect(magnetiDiv).toBeVisible();
    await expect(bulbizarreDiv).not.toBeVisible();
    await expect(piafabecDiv).not.toBeVisible();
});
...
test('maps page has pokemon search', async ({ page }) => {
    ...
    const bulbizarreDiv = page.locator('#pokedex div').filter({ hasText: 'Bulbizarre' });
    const megaDardargnanDiv = page.locator('#pokedex div').filter({ hasText: 'Méga-Dardargnan' });
    await expect(bulbizarreDiv).toBeVisible();
    await expect(megaDardargnanDiv).toBeVisible();

    const searchBar = page.locator('[id="pokemonSearch"]');
    await expect(searchBar).toBeVisible();
    await searchBar.fill('dardargnan');
    await expect(bulbizarreDiv).not.toBeVisible();
    await expect(megaDardargnanDiv).toBeVisible();
});